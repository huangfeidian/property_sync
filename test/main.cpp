...
void test_array(top_msg_queue& cur_top_queue, prop_record_proxy<PropertyMap> test_a_record_proxy, prop_replay_proxy<PropertyMap> test_b_replay_proxy)
{
	mutate_msg msg;

	auto mut_j = test_a_record_proxy.j();

	mut_j.set(std::array<float, 3>{0.1, 0.2, 0.3});
	msg = cur_top_queue.front();
	cur_top_queue.pop_front();
	test_b_replay_proxy.replay(msg.offset.to_replay_offset(), msg.cmd, msg.data);
	if (!(test_a_record_proxy.get_PropertyMap() == test_b_replay_proxy.data()))
...
void test_slots(top_msg_queue& cur_top_queue, prop_record_proxy<PropertyMap> test_a_record_proxy, prop_replay_proxy<PropertyMap> test_b_replay_proxy)
{
	mutate_msg msg;
	// test g begin
	auto mut_h = test_a_record_proxy.h();
	json test_data;
	test_data["a"] = 1;
	test_data["id"] = 2;
	test_data["slot"] = 0;
	mut_h.insert(test_data);
	assert(cur_top_queue.empty());
	mut_h.resize(10);
	...
	test_data["a"] = 1;
	test_data["id"] = 3;
	test_data["slot"] = mut_h.data().get_first_empty_slot();
	mut_h.insert(test_data);
	msg = cur_top_queue.front();
	cur_top_queue.pop_front();
	test_b_replay_proxy.replay(msg.offset.to_replay_offset(), msg.cmd, msg.data);
	...
	auto g_3_proxy_2 = mut_h.get_slot(3);
	if (!g_3_proxy_2)
	{
		std::cout << "fail to relay " << __LINE__ << std::endl;
	}
...
void test_property_vec(top_msg_queue& cur_top_queue, prop_record_proxy<PropertyMap> test_a_record_proxy, prop_replay_proxy<PropertyMap> test_b_replay_proxy)
{
	mutate_msg msg;
	...
	// mutate i begin
	auto mut_i = test_a_record_proxy.i();


	mut_i.set(std::vector<json>{test_data, test_data});
	msg = cur_top_queue.front();
	cur_top_queue.pop_front();
	test_b_replay_proxy.replay(msg.offset.to_replay_offset(), msg.cmd, msg.data);
	if (!(test_a_record_proxy.get_PropertyMap() == test_b_replay_proxy.data()))
...
int main()
{
	...
	test_array(cur_top_queue, test_a_record_proxy, test_b_replay_proxy);
	...
	test_slots(cur_top_queue, test_a_record_proxy, test_b_replay_proxy);
	test_property_vec(cur_top_queue, test_a_record_proxy, test_b_replay_proxy);
